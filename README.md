...
<p align="center">
  <b>虚幻引擎的 agent harness</b><br>
  给一个目标，让 AI 在你正在跑的工程里把活干完 —— 放 Actor、改蓝图、连材质、编译 C++。
</p>
...
一套让 AI Agent 能够实际操作虚幻工程的本地运行环境 —— 模型接入、一百多个虚幻引擎工具、
内置技能、逐步审批与结果核对。Electron + Vue 3 + TypeScript，Apache-2.0。

产品的主体是这个 Agent。项目库、资产库、蓝图库与材质库、笔记与知识库各自有完整的独立界面，
也可以单独使用，但在产品结构中它们服务于 Agent：登记它的作用目标、给它供料、接住它的产出。

本仓库是**社区核心版**：完全离线运行，不需要账号、不依赖官方服务器、没有区域门禁。
装完不配任何东西就能用全部本地功能；AI 功能自带模型 API Key 即可，密钥只存在本机。

不带更新源：包里不写死任何更新服务器，不配就一次网络请求都不发
（见 [updateFeed.ts](src/main/services/updater/updateFeed.ts)）。

连接引擎的 UnrealAgentLink 插件源码位于本仓库的
[`plugin/UnrealAgentLink`](plugin/UnrealAgentLink)，使用独立的 [MIT 许可证](plugin/UnrealAgentLink/LICENSE)。
导入工程时盒子会把随包的预编译版装到
`<项目>/Plugins/UnrealAgentLink/` —— 带二进制是为了让不会编译的用户开箱即用；
想自己编可使用本仓库的构建脚本，工程卡片右键可以移除盒子装的这份。

平台：**Windows 是主力平台**，功能都在这上面验证。macOS / Linux 能构建也能跑起来，
macOS 已支持扫描常见共享安装目录中的引擎，也可手动添加引擎目录或编辑器 `.app`。
macOS 已支持检测运行中的 UE4 / UE5 编辑器及其项目路径；Mac 插件二进制构建与发布验收
仍未完成，Linux 的引擎自动发现尚未适配。

---

## 看它干活

它说做完了不算数 —— 改完回引擎核实一遍，再让它自己举证。这是一轮任务的四个画面：

<!-- TODO: 这里补一段 10 秒 GIF（给目标 → Agent 摆 Actor → 引擎里真的变了），放在 website/public/shots/ 下 -->
...
| ![审查改动](website/public/shots/engine-review.png)<br>**审查改动** —— 机器去引擎里核实后的结论    | ![让它自证](website/public/shots/engine-selfcheck.png)<br>**让它自证** —— 逐条回应查出的问题，说清哪里没做到、哪里是自己加的 |
...
| ![蓝图库](website/public/shots/blueprint-library.png)<br>蓝图库画廊             | ![材质库](website/public/shots/material-library.png)<br>材质库画廊 |
...
下载安装包：[GitHub Releases](https://github.com/ueboxai/uebox/releases) · [官网](https://uebox.ai)

装完打开就能用全部本地功能。要用 AI，去 **设置 → 模型** 填自己的服务商 API Key ——
密钥加密存在本机，不经任何中转。本地推理（Ollama、LM Studio、llama.cpp）和自建网关
（LiteLLM、One API）也在服务商清单里。

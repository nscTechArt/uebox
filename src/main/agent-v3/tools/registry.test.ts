...
/*
 * 「工具定义总量」那道门禁搬走了 → `src/main/agent-v3/core/toolBudget.test.ts`。
 *
 * 它原先用 `字符数 / 3.6` 估算、只量 `buildAllTools()`、阈值 60,000。
 * 2026-09-17 拿真 tokenizer 和真机实测复核，三个方向同时是错的：换算低估中文
 * 1.84 倍、范围漏掉 skill 与系统提示词、而且因为只有中文被低估，它在工具之间的
 * 偏差不均匀（1.33x ~ 2.15x）—— 等于一张会把「描述很长」的工具藏起来的排行榜。
 * 详见新文件的文件头。
 */

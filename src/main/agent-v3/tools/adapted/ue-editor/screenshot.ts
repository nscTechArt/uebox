...
【⚠️ 曝光：先看场景锁没锁】场景用的是**自动曝光**（没放 PostProcessVolume，
或者它的 Metering Mode 不是 Manual）时，这条路自己渲的那一帧、编辑器视口、
跑起来的游戏会各自收敛到不同的亮度，这张图的明暗就没有基准 ——
而且自动曝光会把你压暗的场景重新拉亮，调灯调不出效果。
**锁了曝光**（无边界 PostProcessVolume，Metering Mode = Manual、
Apply Physical Camera Exposure 关掉，再调 Exposure Compensation）之后，
截图、视口、游戏是同一个亮度，这张图的明暗就是玩家看到的。

所以画面过亮、发白、太暗时，**不要把它归给截图偏差然后收工** —— 先查场景
有没有锁曝光：没锁就先锁再调；锁了，那就是场景本身的问题。

在任何情况下都能据此判断：物体在不在、位置对不对、材质什么颜色、
灯有没有亮、阴影方向、构图。
任何情况下都不能据此判断：**界面上有没有东西**（UMG 和屏幕文字根本不画，
见上一条）。

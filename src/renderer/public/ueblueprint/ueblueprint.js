...
class Configuration {
    ...
    static isChinese = false
    static translations = []
    static myTranslationMap = []
    ...
    static contentChangedEventName = "ueb-content-changed"
    ...
    static knotOffset = [-16, -0.5 * Configuration.gridSize]
    ...
    static paths = {
        ...
        materialExpressionNamedRerouteDeclaration: "/Script/Engine.MaterialExpressionNamedRerouteDeclaration",
...
function nodeColor(entity) {
    switch (entity.getType()) {
        ...
        case p$7.materialExpressionNamedRerouteDeclaration: {
            const materialObject = entity.getMaterialSubobject() ?? entity;
            if (materialObject.NodeColor) {
                return materialObject.NodeColor.toCSSRGBValues()
            }
            break
        }
 ...
 * @returns {String}
 */
function nodeTitleForReal(entity) {
    let value;
    switch (entity.getType()) {
        ...
        case p$6.materialExpressionMaterialFunctionCall:
            ...
            break
        case p$6.materialExpressionNamedRerouteDeclaration: {
            const materialObject = entity.getMaterialSubobject() ?? entity;
            return materialObject.DisplayName?.toString() ?? "Named Reroute"
        }
...
// 识别和翻译函数
function translateMember(MemberParent = '/Script/Engine.Actor', MemberName) {

    // console.log(MemberParent, `Key: ${MemberName}`)
    if (!MemberName) {
        return null
    }
    // 步骤2：生成 Key（修改后的逻辑）
    const dotIndex = MemberParent.lastIndexOf('.');
    const keyBase = MemberParent.substring(dotIndex + 1); // 提取点号后的部分

    const key = keyBase ? `${keyBase}:${MemberName}` : MemberName;


    // 步骤3 & 4：查找 Key
    if (Configuration.translations.hasOwnProperty(key)) {
        return Configuration.translations[key]
    } else {
        return null
    }
}
...
function translateText(text) {
    if (Configuration.myTranslationMap.hasOwnProperty(text)) {
        return Configuration.myTranslationMap[text]
    }
}
...
function nodeTitle(entity) {
    ...
    if (Configuration.isChinese) {
        if (entity?.FunctionReference) {
            let title = translateMember(
                entity.FunctionReference?.MemberParent?.path,
                entity?.FunctionReference?.MemberName?.value
            );
            if (title) {
                return title
            }
        } else if (entity?.VariableReference) {
            let title = translateMember(
                entity.VariableReference?.MemberParent?.path,
                entity?.VariableReference?.MemberName?.value
            );
            if (title) {
                return title
            }
        } else if (entity?.EventReference) {
            let title = translateMember(
                entity.EventReference?.MemberParent?.path,
                entity?.EventReference?.MemberName?.value
            );
            if (title) {
                return title
            }
        } else if (entity?.MacroGraphReference) {
            let title = translateText(
                entity.MacroGraphReference?.MacroGraph?.path
            );
            if (title) {
                return title
            }
    ...
    return nodeTitleForReal(entity)
}
...
function pinTitle(entity) {
    ...
    if (match = entity.PinToolTip?.toString().match(/\s*(.+?(?=\n)|.+\S)\s*/)) {
        if (match[1].toLowerCase() === result.toLowerCase()) {
            if (Configuration.isChinese) {
                const title = translatePin(entity);
                if (title) {
                    return title
                }
            }
...
// 识别引脚名称
function translatePin(childEntity) {
    if (!childEntity) {
        return
    }
    const entity = childEntity?.objectEntity;
    let  pinName = childEntity.PinName.value;

    let MemberParent = '/Script/Engine.Actor';
    let MemberName = '';


    if (entity?.FunctionReference) {
        MemberParent = entity.FunctionReference?.MemberParent?.path;
        MemberName = entity?.FunctionReference?.MemberName?.value;
    } else if (entity?.VariableReference) {
        MemberParent = entity.VariableReference?.MemberParent?.path;
        MemberName = entity?.VariableReference?.MemberName?.value;

    } else if (entity?.EventReference) {
        MemberParent = entity.EventReference?.MemberParent?.path;
        MemberName = entity?.EventReference?.MemberName?.value;
    }

    if (!MemberParent || !MemberName) {
        return
    }
    ...
    const baseKey = keyBase ? `${keyBase}:${MemberName}` : MemberName;
    const key = baseKey + ':' + pinName;
...
class ObjectEntity extends IEntity {
    ...
    static displayNameAttributeKey = "DisplayName"
    /** @type {(k: String) => String} */
    static printKey = k => k.startsWith(Configuration.subObjectAttributeNamePrefix)
        ? ""
        : k === "DisplayName"
            ? "Name"
            : k
    ...
    static attributes = {
        ...
        PinTags: ArrayEntity.of(UnknownKeysEntity).flagInlined(),
    ...
    static objectAttributeKey(values, attributeKey) {
        if (
    ...
    static createObjectAttributeGrammar(
        attributeNameGrammar = Parsernostrum.reg(Grammar.Regex.MultipleWordsSymbols),
        valueSeparator = Grammar.equalSeparation,
        handleObjectSet = (values, attributeKey, attributeValue) => { },
    ) {
        return Parsernostrum.seq(
            attributeNameGrammar,
            ...
            const attributeKey = attributeName.split(Configuration.keysSeparator);
            const attributeValue = Grammar.getAttribute(this, attributeKey);
            const grammar = attributeValue ? attributeValue.grammar : IEntity.unknownEntityGrammar;
            const inlined = attributeKey.length > 1;
            return grammar.map(attributeValue =>
                values => {
                    const targetAttributeKey = this.objectAttributeKey(values, attributeKey);
                    Utility.objectSet(values, targetAttributeKey, attributeValue);
                    targetAttributeKey.reduce(
                        (acc, cur, i) => {
                            acc[cur]["inlined"] = inlined && i < targetAttributeKey.length - 1;
                            return acc[cur]
                        },
                        values
                    );
                    handleObjectSet(values, targetAttributeKey, attributeValue);
                }
            )
        })
    }
    ...
    constructor(values = {}) {
        ...
        /** @type {InstanceType<typeof ObjectEntity.attributes.DisplayName>} */ this.DisplayName;
        ...
        /** @type {InstanceType<typeof ObjectEntity.attributes.NodeColor>} */ this.NodeColor;
        ...
        return Parsernostrum.seq(
            ...
            Parsernostrum.seq(
                ...
                Parsernostrum.alt(
                    ...
                    this.createObjectAttributeGrammar(),
                    this.createObjectAttributeGrammar(
...
class IDraggableElement extends IElement {
    ...
    acknowledgeDrag(value) {
        ...
        this.blueprint?.notifyContentChanged();
    }
...
class NodeTemplate extends ISelectableDraggableTemplate {
    ...
    render() {
        return x`
            ...
            ${this.renderNodeComment()}
        `
    }

    renderNodeComment() {
        const comment = this.element.entity.NodeComment?.toString();
        return comment
            ? x`<div class="ueb-node-comment-bubble">${comment}</div>`
            : E
    }
...
class CommentNodeTemplate extends IResizeableTemplate {
    ...
    #focusInput() {
        const input = this.element.querySelector(".ueb-comment-input");
        input?.focus();
    }

    #refreshSelectableAreaHeight() {
        const bounding = this.getDraggableElement()?.getBoundingClientRect();
        this.#selectableAreaHeight = bounding?.height ?? 0;
    }
    ...
    beginEdit = event => {
        ...
        this.#editing = true;
        this.element.requestUpdate();
        this.element.updateComplete.then(() => this.#focusInput());
    }
    ...
    cancelEdit = event => {
        ...
        this.#editing = false;
        this.element.requestUpdate();
        this.element.updateComplete.then(() => this.#refreshSelectableAreaHeight());
    }
    ...
    commitEdit = event => {
        ...
        const input = event?.currentTarget ?? this.element.querySelector(".ueb-comment-input");
        const value = (input?.innerText ?? "")
            .replaceAll("\r\n", "\n")
            .replaceAll("\r", "\n")
            .replace(/(?=\n\s*)\n$/, "");
        if (this.element.entity.NodeComment) {
            this.element.entity.NodeComment.value = value;
        } else {
            this.element.entity.NodeComment = new StringEntity(value);
        }
        this.element.computeNodeDisplayName();
        this.#editing = false;
        this.element.requestUpdate();
        this.element.updateComplete.then(() => {
            this.#refreshSelectableAreaHeight();
            this.element.acknowledgeUpdate();
        });
    }
    ...
    render() {
        const value = this.element.entity.NodeComment?.toString() ?? "";
        return x`
            <div class="ueb-node-border">
                <div class="ueb-node-wrapper">
                    <div class="ueb-node-top" @dblclick="${this.beginEdit}">
                        ${this.#editing
                            ? x`
                                <ueb-input
                                    class="ueb-comment-input"
                                    @focusout="${this.commitEdit}"
                            ...
                            : x`
                                <span
                                    class="ueb-comment-text"
                                    .innerText="${Utility.encodeHTMLWhitespace(value)}">
                                </span>
                            `}
                    </div>
                </div>
...
class MaterialNamedRerouteDeclarationNodeTemplate extends NodeTemplate {

    static nodeStyleClasses = [...super.nodeStyleClasses, "ueb-node-style-material-preview"]

    previewClass() {
        const name = this.element.nodeDisplayName?.toLowerCase() ?? "";
        if (name.includes("angle") || name.includes("\u89d2\u5ea6")) {
            return "ueb-material-preview-angle"
        }
        if (name.includes("radial") || name.includes("\u5f84\u5411")) {
            return "ueb-material-preview-radial"
        }
        return "ueb-material-preview-radial"
    }
        ...
        return x`
            <div class="ueb-node-border">
                <div class="ueb-node-wrapper">
                    ...
                    <div class="ueb-node-inputs"></div>
                    <div class="ueb-node-outputs"></div>
                    <div class="ueb-material-preview ${this.previewClass()}"></div>
                </div>
            </div>
            ${this.renderNodeComment()}
        `
...
function nodeTemplateClass(nodeEntity) {
    ...
    switch (nodeEntity.getType()) {
        case p$1.materialExpressionNamedRerouteDeclaration:
            return MaterialNamedRerouteDeclarationNodeTemplate
    }
...
class BlueprintTemplate extends ITemplate {
    ...
    firstUpdated(changedProperties) {
        super.firstUpdated(changedProperties);
        // Blueprint extends LitElement which implements ParentNode and has querySelector method
        /** @type {HTMLElement} */
        this.headerElement = /** @type {any} */(this.blueprint).querySelector('.ueb-viewport-header');
        /** @type {HTMLElement} */
        this.overlayElement = /** @type {any} */(this.blueprint).querySelector('.ueb-viewport-overlay');
        /** @type {HTMLElement} */
        this.viewportElement = /** @type {any} */(this.blueprint).querySelector('.ueb-viewport-body');
        /** @type {HTMLElement} */
        this.selectorElement = /** @type {any} */(this.blueprint).querySelector('ueb-selector');
        this.gridElement = this.viewportElement.querySelector(".ueb-grid");
        /** @type {HTMLElement} */
        this.linksContainerElement = /** @type {any} */(this.blueprint).querySelector("[data-links]");
        this.linksContainerElement.append(...this.blueprint.getLinks());
        /** @type {HTMLElement} */
        this.nodesContainerElement = /** @type {any} */(this.blueprint).querySelector("[data-nodes]");
...
class Blueprint extends IElement {
    ...
    getSerializedText() {
        const nodes = this.getNodes(true).map(n => n.entity);
        let exports = false;
        let result = nodes
            .filter(n => {
                exports ||= n.exported;
    ...
    /** Serialize ALL nodes (not just selected) */
    getFullSerializedText() {
        const nodes = this.getNodes(false).map(n => n.entity);
        let exports = false;
        let result = nodes
            .filter(n => {
                exports ||= n.exported;
    ...
    /** Notify listeners that the blueprint content has changed */
    notifyContentChanged() {
        this.dispatchEvent(new CustomEvent(Configuration.contentChangedEventName, {
            bubbles: true,
            composed: true,
        }));
    }
    ...
    setChinese(value = true) {
        Configuration.isChinese = value;
        this.nodes.forEach(node => node.acknowledgeUpdate());
    }


    setTranslationsList(list) {
        Configuration.translations = list ?? [];
    }

    setMyTranslationMap(list) {
        Configuration.myTranslationMap = list ?? [];
    }

export default {
  ...
  mcp: {
    ...
    server: {
      title: '共享虚幻引擎能力',
      label: '开放给外部客户端',
      // 第一层只给一行。风险细节在勾「开放写操作工具」时才展开（见 securityNote）
      shortNote: '让 Claude Code、Cursor 直接操作你的引擎。仅本机可连，需令牌，默认只读。',
      securityNote:
      ...
      // 「更改前请先停止服务」不写进常驻描述 —— 控件在运行时本来就点不动，
      // 真去点的时候用 title 说一次就够
      stopToChange: '服务运行中改不了，先关掉上面的开关。',
      port: '端口',
...
可以从这几处看起：这个 .uproject 能不能正常读出来、里面的 EngineAssociation 写的是哪个版本；工程目录下的 Plugins/UnrealAgentLink/ 在不在、里面有没有 UnrealAgentLink.uplugin；这台机器上装没装对应版本的引擎。查清楚之后用一句话告诉我是什么问题。`,
  ...
  profile: {
    ...
    objectStorage: {
      title: '对象存储',
      enable: '启用对象存储',
      enableDesc: '接入你自己的 S3 兼容存储桶。需要把文件放到云上的功能会用它，目前是对话里的视频和音频',
      ...
      empty: '还没有上传过文件',
      listFailed: '读取列表失败',
      removeSelectedTitle: '删除选中的 {count} 个文件？',

...
<template>
  <div class="profile-asset-settings">
    ...
    <!-- Default Apps Group -->
    <div class="settings-group">
      <div class="group-title-collapsible" @click="defaultAppsCollapsed = !defaultAppsCollapsed">
        <PhCaretRight
          weight="fill"
          class="collapse-icon"
          :class="{ collapsed: defaultAppsCollapsed }"
        />
        <h3 class="group-title">{{ $t('profile.asset.defaultApps') }}</h3>
      </div>
